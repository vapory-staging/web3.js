...
The ``web3.vap.subscribe`` function lets you subscribe to specific events in the blockchain.
...
.. code-block:: javascript
    ...
    web3.vap.subscribe(type [, options] [, callback]);
...
.. code-block:: javascript
    ...
    var subscription = web3.vap.subscribe('logs', {
        address: '0x123456..',
        topics: ['0x12345...']
...
.. code-block:: javascript
    ...
    web3.vap.subscribe('logs', {} ,function(){ ... });
...
.. code-block:: javascript
    ...
    web3.vap.subscribe('pendingTransactions' [, callback]);
...
``EventEmitter``: An :ref:`subscription instance <vap-subscription-return>` as an event emitter with the following events:

- ``"data"`` returns ``Object``: Fires on each incoming pending transaction.
- ``"error"`` returns ``Object``: Fires when an error in the subscription occurs.

For the structure of the returned object see :ref:`web3.vap.getTransaction() return values <vap-gettransaction-return>`.
...
.. code-block:: javascript
    ...
    var subscription = web3.vap.subscribe('pendingTransactions', function(error, result){
        if (!error)
            console.log(result);
    })
...
.. code-block:: javascript
    ...
    web3.vap.subscribe('newBlockHeaders' [, callback]);
...
.. code-block:: javascript
    ...
    var subscription = web3.vap.subscribe('newBlockHeaders', function(error, result){
        if (!error)
            console.log(error);
    })
...
.. code-block:: javascript
    ...
    web3.vap.subscribe('syncing' [, callback]);
...
For the structure of a returned event ``Object`` see :ref:`web3.vap.isSyncing return values <vap-issyncing-return>`.
...
.. code-block:: javascript
    ...
    var subscription = web3.vap.subscribe('syncing', function(error, sync){
        if (!error)
            console.log(sync);
    })
...
.. code-block:: javascript
    ...
    web3.vap.subscribe('logs', options [, callback]);
...
For the structure of a returned event ``Object`` see :ref:`web3.vap.getPastEvents return values <vap-getpastlogs-return>`.
...
1. ``Object|Null`` - First parameter is an error object if the subscription failed.
2. ``Object`` - The log object like in :ref:`web3.vap.getPastEvents return values <vap-getpastlogs-return>`.
