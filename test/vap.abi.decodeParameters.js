...
describe('decodeParameters', function () {
    tests.forEach(function (test) {
        it('should convert correctly', function () {
            assert.deepEqual(web3.vap.abi.decodeParameters.apply(web3.vap.abi, test.params), test.result);
        });
    });
});
...
describe('decodeParameters', function () {
    failures.forEach(function (test) {
        it('should not convert '+test.params[1]+' to '+test.params[0], function () {
            assert.throws(_ => {web3.vap.abi.decodeParameters.apply(web3.vap.abi, test.params)});
        });
    });
});
